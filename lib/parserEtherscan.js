...
exports.parseRemapping =
    exports.parseRemappings =
    exports.EtherscanParser =
    exports.networks =
        void 0
...
class EtherscanParser {
    constructor(
        apikey = 'ZAD4UI2RCXCQTP38EXS3UY2MPHFU5H9KB1',
        network = 'mainnet',
        url
    ) {
        this.apikey = apikey
        ...
        if (!exports.networks.includes(network)) {
            throw new Error(
                `Invalid network "${network}". Must be one of ${exports.networks}`
            )
    ...
    async getUmlClasses(contractAddress) {
        ...
        for (const file of files) {
            ...
            const umlClass = (0, converterAST2Classes_1.convertAST2UmlClasses)(
                node,
                file.filename,
                remappings
            )
    ...
    async getSolidityCode(contractAddress, filename) {
        ...
        const topologicalSortedClasses = (0,
        filterClasses_1.topologicalSortClasses)(umlClasses)
        ...
        const solidityVersion = (0, regEx_1.parseSolidityVersion)(
            compilerVersion
        )
    ...
    async getSourceCode(contractAddress, filename) {
        ...
        try {
            ...
            const results = response.data.result.map((result) => {
                ...
                if (result.SourceCode[0] === '{') {
                    try {
                        ...
                        remappings = (0, exports.parseRemappings)(
            ...
            if (filename) {
                files = files.filter(
                    (r) =>
                        path_1.default.parse(r.filename).base == filenameWithExt
                )
                if (!files?.length) {
                    throw new Error(
